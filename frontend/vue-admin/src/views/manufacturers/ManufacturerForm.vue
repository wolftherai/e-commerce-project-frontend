...
name: "ManufacturerForm",
    ...
    async mounted() {
        ...
        {
            const {data} = await this.$http.get(`manufacturers/${this.$route.params.id}`);
    ...
    methods: {
          async cancel() {
            if (confirm('Are you sure? Your changes will be discarded.')){
                await this.$router.push('/manufacturers');
            }
          },
        async submit() {
            const data = {
            ...
            if (this.$route.params.id) { // if id exists
                await this.$http.put(`manufacturers/${this.$route.params.id}`,data) // update product
            }
            else {
                await this.$http.post('manufacturers',data) // create product
            }
